...
app.get('/drama/home',(req,res)=>{
    ...
    const data = async (base_url)=>{
        ...
        // main push
        datas.push({
            slide_img:slide_data,recent_sub,movies
        })
...
app.get('/drama/:drama/:slug',(req,res)=>{
    ...
    const MoreEpisodes = async (as,pages,url) => {
        ...
        const show = await page.evaluate((as,page)=>{
            ...
            var alias = as;
            // return {as:as}
            return $.ajax({url:base_url+"ajax/episode-list/"+alias+"/"+page+".html?page="+page,type:"GET",dataType:"html",success:function(b){
                return b;
            }});
   ...
   const data = async (drama,url)=>{
        ...
        var response = (await fetch(url).then((res)=>res.text()).then((text)=>{return text}).catch(err=>console.log(err)));
...
app.get("/animi/home",(req,res)=>{
    ...
    const FetchData = async () => { 
        ...
        var response = await fetch(url).then(res => res.text()).then(res => {return res}).catch(e=>console.log(e));

        var $ = cheerio.load(response);

        // console.log(response)
        let Recent_Releases = $("#load_recent_release > div.last_episodes.loaddub > ul > li");
        let Popular_Ongoing = $("#load_popular_ongoing > div.added_series_body.popular > ul > li");
        let Recent_Added = $("#wrapper_bg > section > section.content_left > div.main_body.none > div.added_series_body.final > ul > li");
